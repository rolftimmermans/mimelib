 ...
 * @param {Number} [lineMargin]
 * @param {String} [boundary] Line ending boundary (default is \r\n)
 * @return {String} Folded string
 */
this.foldLine = function(str, maxLength, foldAnywhere, afterSpace, lineMargin, boundary){
    if(foldAnywhere){
        return addBase64SoftLinebreaks(str, maxLength || 76, boundary);
    }
    return module.exports.mimeFunctions.foldLine(str, maxLength, !!afterSpace, lineMargin, boundary);
};
...
module.exports.mimeFunctions = {
    ...
    foldLine: function(str, lineLengthMax, afterSpace, lineMargin, boundary){
...
function addBase64SoftLinebreaks(base64EncodedStr, lineLengthMax, boundary){
    base64EncodedStr = (base64EncodedStr || "").toString().trim();
    boundary = boundary || "\r\n"
    return base64EncodedStr.replace(new RegExp(".{" +lineLengthMax+ "}", "g"),"$&" + boundary).trim();
}
